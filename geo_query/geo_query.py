...
@click.option('-e', '--entry', default='gse', type=click.Choice(['gds', 'gpl', 'gse', 'gsm']), help='Type of entry (gds, gpl, gse, gsm).')
@click.option('-m', '--mesh', multiple=True, default=['Diabetes Mellitus, Type 2'], help='Medical Subject Headings (MeSH) terms.')
@click.option('-s', '--sample', multiple=True, default=['rna'], type=click.Choice(['rna', 'mpss', 'sage', 'protein', 'genomic']), help='Type of sample.')
...
def cli(title, description, organism, mesh, date_start, date_end, sample, entry):
    ...
    search_term.append(build_query(terms=sample, query_type='Sample Type', operator='OR'))
    search_term.append(build_query(terms=mesh, query_type='MeSH Terms', operator='OR'))
    if description:
        search_term.append(build_query(terms=description, query_type='Description', operator='OR'))
    if title:
